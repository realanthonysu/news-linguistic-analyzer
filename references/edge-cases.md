...
| 超长文本（>2000 词） | 按「超长文本处理」规则执行主题感知分段（见 long-text-processing.md） |
...
| web search 不可用 | 使用 web fetch 访问已知权威新闻源页面获取信息 |
| web search 和 web fetch 均不可用 | 标注「⚠️ 未能联网验证，以下分析基于模型内置知识」，继续后续步骤 |
